from text_matching.dssm.graph import Graph
import tensorflow as tf
from text_matching.dssm.utils import load_char_data
from text_matching.dssm import args

p, h, y = load_char_data('../text_matching/input/train.csv', data_size=None)
p_eval, h_eval, y_eval = load_char_data('../text_matching/input/dev.csv', data_size=None)

p_holder = tf.placeholder(dtype=tf.int32, shape=(None, args.seq_length), name='p')  #(?,15)
h_holder = tf.placeholder(dtype=tf.int32, shape=(None, args.seq_length), name='h')  #(?,15)
...
dataset = dataset.batch(args.batch_size).repeat(args.epochs)  # (100) -> (5)

...
class Graph:
    ...
    def BiLSTM(self, x):
        fw_cell = tf.nn.rnn_cell.BasicLSTMCell(args.lstm_hidden)  # 100
    ...
    def forward(self):
        ...
        same_word = tf.tile(same_word, [1, 15, 1])  # shape=(?, 15, 1)

        # [None, 15, 301]
        p = tf.concat((p_char_embedding, p_word_embedding, self.p_w_vec, same_word), axis=-1)  # shape=(?, 15, 301)
        ...
        for i in range(4):
            ...
            for j in range(5):
                with tf.variable_scope(f'p_lstm_{i}_{j}', reuse=None):
                    p_state, _ = self.BiLSTM(tf.concat(p_state, axis=-1))  # shape=(?, 15, 100)

                with tf.variable_scope(f'p_lstm_{i}_{j}' + str(i), reuse=None):
                    h_state, _ = self.BiLSTM(tf.concat(h_state, axis=-1))   # shape=(?, 15, 100)

                p_state = tf.concat(p_state, axis=-1)  # shape=(?, 15, 200)
                h_state = tf.concat(h_state, axis=-1)

                # attention
                cosine = tf.divide(tf.matmul(p_state, tf.matrix_transpose(h_state)),  #shape=(?, 15, 15)
                                   (tf.norm(p_state, axis=-1, keep_dims=True) * tf.norm(h_state, axis=-1,
                                                                                        keep_dims=True)))

                att_matrix = tf.nn.softmax(cosine)
                p_attention = tf.matmul(att_matrix, h_state)  # shape=(?, 15, 200)
                h_attention = tf.matmul(att_matrix, p_state)

                # DesNet   shape=(?, 15, 301) shape=(?, 15, 200)  shape=(?, 15, 200)
                p = tf.concat((p, p_state, p_attention), axis=-1)  #shape=(?, 15, 701)
        ...
        add = p + h
        sub = p - h
        norm = tf.norm(sub, axis=-1)    #shape=(?, 15)

...
class Graph:
    def __init__(self, word_embedding=None):
        ...
        self.gate_w2 = tf.get_variable(name='gate_w2', shape=(args.d * 2, args.d))  # d = 100
    ...
    def dense_net(self, v):
        filters = args.d * args.dense_eta    #  d=100 dense_eta = 0.3  dense_g = 20

        v_in = tf.layers.conv2d(v, filters=filters, kernel_size=(1, 1))  # (?,,30)
        for _ in range(3):
            for _ in range(8):
